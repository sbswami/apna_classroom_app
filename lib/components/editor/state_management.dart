...
class EditorProvider extends ChangeNotifier {
  ...
  EditorProvider({SmartTextType defaultType = SmartTextType.T, data}) {
    if (data == null) {
    ...
    data.asMap().forEach((index, value) {
      insert(
          index: index,
          text: value[C.DATA],
          type: getSmartTextTypeFromString(value[C.DATA_TYPE]));
    });
  }
  ...
  List<Map<String, dynamic>> getString() {
    int contentCount = _text.length;
    List<Map<String, dynamic>> content = [];
    for (int index = 0; index < contentCount; index++) {
      content.add({
        C.DATA: _text[index].text,
        C.DATA_TYPE: _types[index].toShortString(),
      });
    }
    return content;
    // print(_text.map((e) => e.text).toList().join('\n'));
  }
}
